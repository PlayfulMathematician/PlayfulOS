...
bool interrupt_register(uint8_t int_no, interrupt_handler_t handler, void *data) {
...
void isr_handler(struct interrupt_context *ctx) {
    ...
    if (int_no < MAX_HANDLERS && handlers[int_no].active && handlers[int_no].handler != NULL) {
